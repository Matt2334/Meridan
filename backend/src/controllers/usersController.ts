...
const deleteUser = async (
  ...
  try {
    ...
    if (!user) return res.status(404).json({ error: "User not found" });
    await Prisma.sessionItem.deleteMany({ where: { session: { userId } } });
    await Prisma.sessionConnection.deleteMany({
      where: { fromSession: { userId } },
    });
    await Prisma.session.deleteMany({ where: { userId } });
    await Prisma.bookmark.deleteMany({ where: { userId } });
    await Prisma.user.delete({ where: { id: userId } });
    await deleteCachePattern(cacheKeys.sessionsAll(userId));
    await deleteCache(cacheKeys.profile(userId));
    res.clearCookie("token");
...
const updateUser = async (
  ...
  try {
    ...
    if (!user) return res.status(404).json({ error: "User not found" });
    await setCache(cacheKey, user, 300);
    return res.status(200).json(user as UpdateUserResponse);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: (err as Error).message });
  }
